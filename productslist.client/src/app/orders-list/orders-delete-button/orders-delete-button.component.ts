...
@Component({
  standalone: true,
  selector: 'orders-delete-button',
  templateUrl: './orders-delete-button.component.html',
  styleUrls: ['./orders-delete-button.component.css'],
  imports: [FormsModule, CommonModule]
})
export class OrdersDeleteButtonComponent implements OnInit, OnDestroy {
  @Input() id: number = 0;
  ...
  delete(url: string, id: number) {
    ...
    this.subscription = delete$.subscribe({
      next: () => {
        ...
        console.log("удаление заказа " + (id.toString() + "..."));
      },
      error: (error: any) => {
        this.status = "fail";
